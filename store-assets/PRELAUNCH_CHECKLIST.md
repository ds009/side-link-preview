...
- [x] `manifest.json` `version` = `1.1.0` — matches the tag you'll publish.
...
- [x] `npm run validate:locales` confirms all 9 locale files share keys
      (75/75).

## 2. Build artifact

- [x] `npm run zip` is rewritten in `scripts/zip.mjs`; produces a clean
      `dist/side-link-preview-1.1.0.zip` (~108 KB, well under 10 MB).
...
- [ ] **[manual]** `git tag v1.1.0 && git push origin v1.1.0`.
